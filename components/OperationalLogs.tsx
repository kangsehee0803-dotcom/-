...
const OperationalLogs: React.FC = () => {
  ...
  const characterThoughts: Record<string, string[]> = {
    ...
    'CAFFEINE': [
      ...
      "마르코, 제발 폭파할 때 파편 좀 신경 써요. 치료해 주기 귀찮으니까.",
  ...
  const initialLogs: LogEntry[] = [
    ...
    { id: 5, time: '22:45:31', sender: 'CAFFEINE', message: '마르코, 심박수 너무 높아요. 진정해요. 데이터 꼬이면 책임 안 집니다.', type: 'COMMS' },
      ...
      <div className="bg-deepbase border border-nebula/40 shadow-[0_0_60px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col h-[500px] md:h-[700px] rounded-lg relative">
        ...
        >
          ...
          <div className="animate-pulse flex space-x-4 items-center pl-4 border-l border-transparent">
             ...
             <div className="flex space-x-1">
               <div className="w-1.5 h-1.5 bg-mint/40 rounded-full animate-bounce" style={{animationDelay: '0s'}}></div>
               <div className="w-1.5 h-1.5 bg-mint/40 rounded-full animate-bounce" style={{animationDelay: '0.2s'}}></div>
               <div className="w-1.5 h-1.5 bg-mint/40 rounded-full animate-bounce" style={{animationDelay: '0.4s'}}></div>
             </div>
             <span className="text-nebula/40 font-mono text-[10px] uppercase tracking-widest font-black">Intercepting Comms...</span>
          </div>
        </div>
        ...
        <div className="p-6 bg-darkteal/20 border-t border-nebula/20 shrink-0 relative z-10">
          <div className="flex space-x-4">
            ...
            <button 
              onClick={startChat}
              className="px-10 py-4 bg-nebula text-offwhite text-xs font-mono font-black hover:bg-mint hover:text-deepbase transition-all uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(112,0,255,0.3)] active:scale-95 rounded-sm"
            >
              채팅 시작
            </button>
          </div>
